...
async def get_flowise_executions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get Flowise executions: {str(e)}"
        )


@router.post("/executions")
async def create_flowise_execution(execution_data: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create Flowise execution: {str(e)}"
        )
...
async def get_workflow_executions(
    ...
    try:
        executions = (
            db.query(FlowiseExecution)
            .filter(FlowiseExecution.workflow_id == workflow_id)
            .order_by(FlowiseExecution.timestamp.desc())
            .limit(limit)
            .all()
        )
...
async def receive_langchain_run(
    ...
    try:
        ...
        background_tasks.add_task(
            process_flowise_run_background, run_data, evaluation_run_id, db
        )
        ...
        return {"success": False, "error": f"Failed to process LangChain run: {str(e)}"}
...
async def receive_evaluation_metrics(
    ...
    try:
        ...
        background_tasks.add_task(
            process_flowise_metrics_background, evaluation_run_id, metrics, db
        )
...
async def process_flowise_run_background(
    run_data: Dict[str, Any], evaluation_run_id: str, db: Session
):
    ...
    try:
        ...
        print(
            f"Successfully processed Flowise run {run_data.get('id')} -> trace {trace_id}"
        )
...
async def process_flowise_metrics_background(
    evaluation_run_id: str, metrics: List[str], db: Session
):
    ...
    try:
        ...
        print(
            f"Successfully processed {len(metrics)} metrics for evaluation {evaluation_run_id}"
        )
