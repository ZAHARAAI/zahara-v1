...
async def get_traces(
    ...
    limit: int = Query(25, ge=1, le=100, description="Alias for page_size (backward compatibility)"),
    status: Optional[str] = Query(None, description="Comma-separated list of statuses (OK/ERROR/RATE-LIMIT)"),
    model: Optional[str] = Query(None, description="Comma-separated list of models to filter by"),
    models: Optional[str] = Query(None, description="Alias for model (backward compatibility)"),
    ...
    sort_by: str = Query("timestamp", description="Field to sort by (timestamp/duration/tokens/cost/status)"),
...
async def get_trace(
    ...
    try:
        ...
        if trace.spans:
            ...
            avg_duration = sum(s.duration for s in trace.spans) / total_spans if total_spans > 0 else 0
            ...
            trace_dict['aggregate_metrics'] = {
                ...
                'rate_limit_count': len([s for s in trace.spans if s.status == 'RATE-LIMIT'])
            }
...
async def get_trace_spans(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get trace spans: {str(e)}")
...
async def export_traces_get(
    ...
    operations: Optional[str] = Query(None, description="Comma-separated operation filters"),
...
async def create_trace_internal(
    trace_data: dict,
    db: Session = Depends(get_db)
):
