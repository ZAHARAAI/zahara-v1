import os
import uuid
from sqlalchemy import Column, String, Integer, Float, DateTime, Text, Boolean, ForeignKey, Index, Numeric, TypeDecorator
...
class Trace(Base):
    ...
    flowise_executions = relationship("FlowiseExecution", back_populates="trace", cascade="all, delete-orphan")
...
class Span(Base):
    ...
    events = relationship("TraceEvent", back_populates="span", cascade="all, delete-orphan")
...
class FlowiseExecution(Base):
    ...
    flowise_data = Column(JSON(), nullable=True)  # Store LangChain Run data, token usage, etc.
