...
from ..models.trace import FlowiseExecution, Span, Trace, TraceEvent
...
class TraceService:
    ...
    def get_dashboard_metrics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            base_query = self.db.query(Trace).filter(
                Trace.timestamp >= start_time, Trace.timestamp <= end_time
            )
            ...
            success_rate = (
                (success_count / total_traces * 100) if total_traces > 0 else 0
            )
            ...
            rate_limit_rate = (
                (rate_limit_count / total_traces * 100) if total_traces > 0 else 0
            )
            ...
            metrics = (
                self.db.query(
                    ...
                    .within_group(Trace.total_duration)
                    .label("p50_latency"),
                ...
                .filter(Trace.timestamp >= start_time, Trace.timestamp <= end_time)
                .first()
            )
            ...
            return {
                ...
                "avg_latency": float(metrics.avg_latency / 1000)
                if metrics.avg_latency
                else 0,  # Convert to seconds
                "p50_latency": float(metrics.p50_latency / 1000)
                if metrics.p50_latency
                else 0,
                "p95_latency": float(metrics.p95_latency / 1000)
                if metrics.p95_latency
                else 0,
                "success_rate": round(success_rate, 1),
                "error_rate": round(error_rate, 1),
                "rate_limit_rate": round(rate_limit_rate, 1),
                "total_tokens_24h": int(metrics.total_tokens)
                if metrics.total_tokens
                else 0,
                "total_cost_24h": float(metrics.total_cost)
    ...
    def export_traces_csv(
        ...
        **filters,
        ...
        try:
            ...
            if include_metadata:
                headers.extend(
                    ["request_id", "client_ip", "user_agent", "trace_metadata"]
                )
                        ...
                        for i in range(max_items):
                            ...
                            if include_spans:
                                if i < len(spans):
                                    ...
                                    if include_metadata:
                                        row_data.append(
                                            json.dumps(span.span_metadata)
                                            if span.span_metadata
                                            else ""
                                        )
                                else:
                                    span_columns = 8 + (1 if include_metadata else 0)
    ...
    def get_flowise_executions(self, limit: int = 100) -> List[FlowiseExecution]:
        ...
        try:
            executions = (
                self.db.query(FlowiseExecution)
                .order_by(desc(FlowiseExecution.timestamp))
                .limit(limit)
                .all()
            )
