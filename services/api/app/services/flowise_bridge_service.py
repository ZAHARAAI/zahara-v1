...
class FlowiseBridgeService:
    ...
    def process_langchain_run(self, run_data: Dict[str, Any], evaluation_run_id: str) -> str:
        """
        ...
        try:
            ...
            tokens, cost, prompt_tokens, completion_tokens = self._extract_token_usage_from_run(run_data, model)
            ...
            if start_time and end_time:
                ...
                span = self.trace_service.add_span(
                    trace_id=trace_id,
                    span_id=span_id,
                    operation=operation,
                    model=model,
            ...
            flowise_execution = self.trace_service.create_flowise_execution(
                workflow_id=evaluation_run_id,
                trace_id=trace_id,
                flowise_data={
                    'run_id': run_id,
    ...
    def process_evaluation_metrics(self, evaluation_run_id: str, metrics: List[str]) -> None:
        """
        ...
        try:
            ...
            traces = self.db.query(Trace).filter(
                Trace.workflow_id == evaluation_run_id
            ).all()
            ...
            for trace in traces:
                for metric_json in metrics:
                    try:
                        ...
                        if 'completionTokens' in metric_data and 'promptTokens' in metric_data:
                            total_tokens = metric_data.get('completionTokens', 0) + metric_data.get('promptTokens', 0)
    ...
    def _process_child_run(self, child_run: Dict[str, Any], parent_trace_id: str, evaluation_run_id: str) -> None:
    ...
    def _extract_model_name_from_run(self, run_data: Dict[str, Any]) -> str:
        ...
        model = (
            ...
            extra.get('metadata', {}).get('ls_model_name') or
            extra.get('metadata', {}).get('fw_model_name')
        )
    ...
    def _extract_token_usage_from_run(self, run_data: Dict[str, Any], model: str) -> tuple[int, float, int, int]:
        """
        ...
        elif (outputs.get('generations', []) and 
              len(outputs['generations']) > 0 and 
              len(outputs['generations'][0]) > 0 and 
              outputs['generations'][0][0].get('message', {}).get('usage_metadata', {}).get('total_tokens')):
        ...
        # Method 3: Fallback token counting (simplified version of EvaluationRunTracer logic)
            ...
            if 'inputs' in run_data and 'outputs' in run_data:
                ...
                completion_tokens = self._estimate_completion_tokens(run_data.get('outputs', {}))
            ...
            for generation_list in outputs['generations']:
                if isinstance(generation_list, list):
                    for generation in generation_list:
                        ...
                        elif 'message' in generation and 'content' in generation['message']:
    ...
    def _extract_message_content(self, message: Dict[str, Any]) -> str:
        ...
        return (
            ...
            message.get('SystemMessage', {}).get('content') or
            message.get('HumanMessage', {}).get('content') or
            message.get('AIMessage', {}).get('content') or
            ''
        )
    ...
    def _calculate_cost(self, metric_data: Dict[str, Any]) -> float:
        ...
        if 'totalTokens' in metric_data and 'model' in metric_data:
            return self._calculate_cost_for_model(
                metric_data['model'], 
                metric_data['totalTokens']
            )
        return 0.0
    
    def _determine_operation_from_run(self, run_data: Dict[str, Any], run_type: str) -> str:
