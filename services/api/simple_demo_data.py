...
def create_simple_demo_data():
    ...
    try:
        ...
        for i, trace_info in enumerate(traces_data * 7):  # Create 35 traces
            ...
            for j in range(3):  # 3 spans per trace
                ...
                span = Span(
                    ...
                    tokens=int(trace_info["tokens"] / 3) if trace_info["tokens"] > 0 else 0,
                    cost=Decimal(str(trace_info["cost"] / 3)) if trace_info["cost"] > 0 else Decimal("0.0"),
        ...
        session.commit()
        print(f"✅ Successfully loaded demo data!")
